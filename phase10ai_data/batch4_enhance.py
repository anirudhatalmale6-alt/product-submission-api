#!/usr/bin/env python3
"""
Phase 10AI Batch 4: Content Enhancement for Dog Grooming, Dog Harnesses,
Dog Beds, Educational, Uncategorized clusters.

Adds: At a Glance, Why This Matters, What We Considered (buying guides only),
...
# ── Content generation maps ─────────────────────────────────────────────────
# Each post gets unique, topic-specific content for every block.

def generate_at_a_glance(post_id, title, cluster):
    """Generate 3-5 bullet points specific to each post."""
    glance = {
        # Dog Grooming
...
def generate_why_matters(post_id, title, cluster):
    """Generate 1-2 sentences of UK-focused context."""
    matters = {
        # Dog Grooming
...
def generate_what_we_considered(post_id, title, cluster):
    """Generate 2-3 sentences about evaluation criteria. Only for buying guides."""
    considered = {
...
def generate_when_to_seek_help(post_id, title, cluster):
    """Generate 1-2 sentences about when to consult a professional."""
    seek = {
        # Dog Grooming
...
def get_trust_footer(cluster, post_id, title):
    """Build cluster-appropriate trust footer."""
    # Determine which references to emphasise based on cluster and topic
    ...
    if cluster == "Dog Grooming":
    ...
    else:  # Uncategorized - topic-appropriate
        if "bed" in title_lower or "wash" in title_lower or "place" in title_lower:
            refs = 'We reference UK veterinary and welfare organisations including the <a href="https://www.pdsa.org.uk/" rel="nofollow">PDSA</a> and <a href="https://www.bva.co.uk/" rel="nofollow">BVA</a>.'
        elif "first aid" in title_lower or "safety" in title_lower or "seasonal" in title_lower:
            refs = 'We reference UK veterinary and welfare organisations including the <a href="https://www.rspca.org.uk/" rel="nofollow">RSPCA</a>, <a href="https://www.pdsa.org.uk/" rel="nofollow">PDSA</a>, and <a href="https://www.bva.co.uk/" rel="nofollow">BVA</a>.'
        ...
        elif "multi-pet" in title_lower or "pet" in title_lower:
            refs = 'We reference UK veterinary and welfare organisations including the <a href="https://www.rspca.org.uk/" rel="nofollow">RSPCA</a>, <a href="https://www.pdsa.org.uk/" rel="nofollow">PDSA</a>, and <a href="https://www.bva.co.uk/" rel="nofollow">BVA</a>.'
        else:
            refs = 'We reference UK veterinary and welfare organisations including the <a href="https://www.rspca.org.uk/" rel="nofollow">RSPCA</a>, <a href="https://www.pdsa.org.uk/" rel="nofollow">PDSA</a>, and <a href="https://www.bva.co.uk/" rel="nofollow">BVA</a>.'
...
def api_post(endpoint, data):
    """POST to WP API using curl with temp file."""
    url = f"{API_BASE}/{endpoint}"
...
def has_block(content, marker):
    """Check if content already contains a specific block marker."""
    return marker.lower() in content.lower()


def is_buying_guide(title):
    """Determine if this is a 'Best X' buying guide."""
    title_lower = title.lower()
    return title_lower.startswith("best ") or "buying guide" in title_lower


def find_first_paragraph_end(content):
    """Find the end of the first content paragraph."""
    # Look for end of first wp:paragraph block
...
def find_faq_position(content):
    """Find the start of FAQ section."""
    # Look for FAQ heading
    patterns = [
        r'<h[23][^>]*>.*?(?:FAQ|Frequently Asked)',
        r'<!-- wp:heading.*?-->.*?(?:FAQ|Frequently Asked)',
    ]
    for pat in patterns:
        match = re.search(pat, content, re.IGNORECASE)
        if match:
            # Find the wp:heading comment before this
            before = content[:match.start()]
            last_heading = before.rfind('<!-- wp:heading')
            if last_heading >= 0:
                return last_heading
            return match.start()
    return None
...
def find_trust_footer(content):
    """Find existing trust footer section to replace."""
    # Look for editorial standards / trust section markers
    patterns = [
        # New format
        r'<!-- wp:group.*?-->.*?Our Editorial Standards.*?<!-- /wp:group -->',
        # Old format patterns
        r'<!-- wp:group.*?-->.*?editorial process.*?affiliate disclosure.*?<!-- /wp:group -->',
        r'<!-- wp:group.*?-->.*?Editorial Standards.*?<!-- /wp:group -->',
        r'<!-- wp:group.*?-->.*?editorial-process.*?<!-- /wp:group -->',
        r'<!-- wp:group.*?-->.*?How We Create Content.*?<!-- /wp:group -->',
        r'<!-- wp:group.*?-->.*?how we create our content.*?<!-- /wp:group -->',
        r'<!-- wp:separator.*?-->.*?<!-- /wp:separator -->.*?<!-- wp:paragraph.*?-->.*?<p[^>]*>.*?editorial.*?</p>.*?<!-- /wp:paragraph -->',
    ]
    for pat in patterns:
    ...
    # Fallback: look for the last wp:group that mentions editorial/affiliate
    last_group_start = None
    last_group_end = None
    for match in re.finditer(r'<!-- wp:group[^>]*-->.*?<!-- /wp:group -->', content, re.DOTALL):
        text = match.group().lower()
        if 'editorial' in text or 'affiliate' in text or 'how we create' in text:
            last_group_start = match.start()
            last_group_end = match.end()
    if last_group_start is not None:
        return last_group_start, last_group_end

    return None, None


def find_end_position(content):
    """Find position near end of content for inserting blocks before trust footer."""
    trust_start, _ = find_trust_footer(content)
    if trust_start:
        return trust_start
    # Otherwise, end of content
    return len(content)
...
def enhance_post(post_id, title, cluster):
    """Fetch, enhance, and update a single post."""
    blocks_added = []
    ...
    # 2. WHY THIS MATTERS - after At a Glance (or after 2nd paragraph)
    if not has_block(content, "Why this matters"):
        text = generate_why_matters(post_id, title, cluster)
        block = build_why_matters(text)
        # Insert after At a Glance if it exists, else after 2nd paragraph
        aag_end = content.find("At a Glance")
        if aag_end >= 0:
            # Find end of the At a Glance group
        ...
        if considered_text:
            ...
            if wtm_pos >= 0:
                ...
                if group_end >= 0:
                    ...
                    insert_pos = find_first_paragraph_end(content) + 200
            else:
                insert_pos = find_first_paragraph_end(content) + 200
            content = content[:insert_pos] + "\n\n" + block + "\n\n" + content[insert_pos:]
    ...
    if not has_block(content, "Troubleshooting Common Issues"):
        ...
        if faq_pos:
            ...
            # Insert before trust footer area
    ...
    if not has_block(content, "When to seek professional help"):
        ...
        # Insert after troubleshooting section
        trouble_pos = content.find("Troubleshooting Common Issues")
        if trouble_pos >= 0:
            # Find end of the troubleshooting paragraph
            para_end = content.find("<!-- /wp:paragraph -->", trouble_pos)
            if para_end >= 0:
    ...
    trust_start, trust_end = find_trust_footer(content)
    ...
    if trust_start is not None:
        ...
        # Append trust footer at the end
...
def main():
    ...
    print(f"Phase 10AI Batch 4: Enhancing {total} posts across 5 clusters")
    ...
    print(f"\n{'=' * 70}")
    print(f"COMPLETE: {done} posts processed, {errors} errors")
